...
Do not build a Vite/React mock of TomorrowOS ‚Ä?use @tomorrowos/sdk cms-starter (Node + WebSocket).
Question 3 only updates brand.json.
After setup questions (including Cloudinary Secrets if chosen), configure and deploy ‚Ä?minimal verification only.
Never test Supabase connection ‚Ä?save SUPABASE_URL and move on. No Supabase or WebSocket tests.
Do not skip steps.
```

Alternate triggers that also activate this protocol:

- ‚ÄúSet up TomorrowOS on Lovable using LOVABLE_SETUP.md‚Ä?
- ‚ÄúConfigure my TomorrowOS CMS (Lovable wizard)‚Ä?

When any of these triggers fire, **ignore `LLM_PROMPT.md` entirely** for the Q&A phase.

**Before Question 1:** read **IRON RULE: real Node CMS only** below. If the project is already a default Lovable Vite + React app with no `@tomorrowos/sdk` server, **do not** keep building that as the TV CMS ‚Ä?seed or replace with the SDK `cms-starter` layout first.
...
Lovable‚Äôs default app is **Vite + React + Lovable Cloud** (Postgres / Auth / Storage / Edge Functions). That stack **cannot** replace `@tomorrowos/sdk`‚Äôs long-lived WebSocket CMS by itself.
...
| Live Node entry: `npm run start` ‚Ü?`tsx server.ts` | Claiming Lovable Cloud **Publish** alone is the TV CMS endpoint |
| Control Panel HTML at `/` via `staticRoot: public` | Vite-only SPA that ‚Äúlooks like‚Ä?a CMS but has no TomorrowOS server |
...
1. **Lovable Agent** = wizard + editor (ask Q1‚ÄìQ3, write Secrets / `brand.json` / `server.ts`).
...
3. **TV-facing CMS host** (pick one; do not invent a fourth):
   - **Railway** (recommended companion) ‚Ä?long-lived Node, `npm run start`
   - **Render / Fly.io** ‚Ä?same pattern as Railway
   - **Vercel Fluid** ‚Ä?only if following **`VERCEL_SETUP.md`** / `cms-starter-v0` (do not mix Replit-style root `server.ts` with broken Vercel static deploy)

**Do not tell the user ‚ÄúPublish on Lovable Cloud‚Ä?is done** unless the **Node CMS URL** (Railway / Render / Fly / Vercel Fluid) is live and serves the Control Panel at `/`.

Lovable Cloud remains useful for:

- **Secrets** UI
- **Postgres** (same family as Supabase ‚Ä?use as `SUPABASE_URL` / database URL)
- **Lovable Cloud Storage** (Object Storage replacement ‚Ä?see Question 2)
...
| Step | Section | When |
|------|---------|------|
| 1 | **Question 1** ‚Ä?Supabase / Lovable Cloud Postgres connection string | Always |
| 2 | **Question 2** ‚Ä?Media storage (+ Cloudinary Secrets if chosen) | Always (after Q1) |
| 3 | **Question 3** ‚Ä?Brand / TomorrowOS app look | Always last, before execution |
...
**Critical:** Question 2 is **not complete** when the user says ‚ÄúCloudinary‚Ä?or ‚Äúyes‚Ä? You **must** immediately collect `CLOUDINARY_CLOUD_NAME`, `CLOUDINARY_API_KEY`, and `CLOUDINARY_API_SECRET` **before** Question 3. Do **not** skip to branding with placeholder or invented credentials.

If the user volunteers extra info early (e.g. brand name before Q3), **record it** and still ask the current question‚Äôs required fields you do not yet have. Do not skip ahead to execution until Q1‚ÄìQ3 are complete (Question 2 includes Cloudinary Secrets when applicable).

If the user says ‚Äújust set it up, don‚Äôt ask questions,‚Ä?respond:

> I need a few quick answers from LOVABLE_SETUP.md (Postgres connection string, media storage ‚Ä?plus Cloudinary credentials if you choose Cloudinary ‚Ä?and branding). It takes about a minute and ensures the CMS deploys correctly with Lovable + a Node host.

Then ask **Question 1** ‚Ä?do not switch to `LLM_PROMPT.md`.

---

## Hard rules

1. **Ask only LOVABLE_SETUP.md questions 1‚Ä?.** Never use `LLM_PROMPT.md` on Lovable for this wizard.
2. **One question at a time.** Wait for the user‚Äôs answer before asking the next (unless they already answered several in one message). **Exception:** If the user chooses Cloudinary in Question 2, **stay on Question 2** and immediately collect Cloudinary Secrets ‚Ä?do **not** announce a separate ‚Ä?b‚Ä?
...
6. **Never commit secrets.** Put credentials in Lovable **Cloud ‚Ü?Secrets** (and the Node host‚Äôs env vars: Railway / Render / Fly / Vercel).
7. After Q&A, **configure and deploy** ‚Ä?do **not** run a long test suite (see **Post-Q&A: minimal verification only**).
8. **Supabase / Lovable Postgres: configure only ‚Ä?never test.** Save `SUPABASE_URL` + `TOMORROWOS_STORE=supabase` and proceed. Do not ping Postgres or treat preview DB errors as a failed setup.
...
## Question 1 ‚Ä?Supabase / Lovable Cloud Postgres (always)
...
>
> If you use **Lovable Cloud**, open **Cloud ‚Ü?Database** (or project connection settings) and copy the Postgres URI. Also confirm SSL (usually **yes**).
...
3. Mirror the same env vars on the **Node host** (Railway / Render / Fly / Vercel) ‚Ä?Lovable Secrets alone do not reach a Railway process.
...
6. **Do not** test the database connection after saving ‚Ä?configuration only.
...
>
> **Recommended: Cloudinary** ‚Ä?public HTTPS URLs; `@tomorrowos/sdk` auto-detects `CLOUDINARY_*` Secrets and `/media/upload` works out of the box.
>
> Alternatives:
> - **Lovable Cloud Storage** (replaces Replit Object Storage) ‚Ä?public Storage bucket on Lovable Cloud / Supabase Storage; durable HTTPS URLs for players.
> - **Local disk only** ‚Ä?`public/uploads` on the Node host (fine for quick tests; files may disappear on redeploy without a volume).
>
> Do you want me to set up **Cloudinary**? (yes / no ‚Ä?if no, say whether you want **Lovable Cloud Storage** or local only)

### Step B ‚Ä?If YES / Cloudinary (same Question 2 ‚Ä?no ‚Ä?b‚Ä?

**Do this immediately** when the user chooses Cloudinary. **Go straight to collecting Secrets.**

**Preferred:** Lovable **Cloud ‚Ü?Secrets** fields for:
...
> Paste your Cloudinary credentials (from [cloudinary.com/console](https://cloudinary.com/console) ‚Ü?Dashboard ‚Ü?**API Keys**). I will store them as Secrets:
>
...
4. **Do not** run a Cloudinary upload test ‚Ä?configuration only.

The SDK auto-detects these env vars; new uploads return `https://res.cloudinary.com/...` URLs.

### If NO ‚Ä?Lovable Cloud Storage (Object Storage replacement)
...
3. Save Secrets (Lovable + Node host) as needed for Storage access used by your upload path, typically:
   - `SUPABASE_URL` (already from Q1 ‚Ä?API URL form may differ from Postgres URI; if the project exposes a separate API URL, store it as `SUPABASE_API_URL` only when required)
...
## Question 3 ‚Ä?Brand / TomorrowOS app look (`brand.json` only)
...
> Let‚Äôs brand your TomorrowOS experience. Please provide:
>
...
> 7. **Logo** ‚Ä?upload an SVG/PNG into the project, or give a URL I can fetch into `./assets/`
...
- Set `cmsEndpoint` only if the user already knows the public `wss://‚Ä¶` URL; otherwise leave it out and tell them to point players at the published HTTPS CMS URL (player converts `https://` ‚Ü?`wss://`).
...
4. Paste Secrets from Q1‚ÄìQ2 into the host.
5. Deploy and open the public HTTPS URL.

**If the user insists on Vercel:** stop using this Railway-oriented finish path and follow **`VERCEL_SETUP.md`** (`npx tomorrowos init --hosting v0`) instead ‚Ä?do not half-migrate.
...
3. Deploy Node host ‚Ü?Preview Control Panel at `/`.

### Do NOT run after setup (unless user asks or deploy fails)

- ‚ù?Supabase connection test
- ‚ù?Cloudinary upload test
- ‚ù?Formal WebSocket console gates
- ‚ù?Long troubleshooting when `/` already shows the Control Panel

---

## What not to ask / change

- **`LLM_PROMPT.md`** ‚Ä?not used for this Lovable wizard
- Standalone screen-count / platform / hosting questionnaires outside Q1‚ÄìQ3
...
## SQLite fallback ‚Ä?only if user refuses Postgres
...
|---------|--------------|-----|
| ‚ÄúCMS‚Ä?is a React app with no pairing | Agent built a Lovable mock | Re-seed with `npx @tomorrowos/sdk init`; deploy Node host |
| Lovable Publish URL does not pair TVs | Expected ‚Ä?no WebSocket CMS there | Deploy Railway / Render / Fly / Vercel Fluid; use that URL |
...
## Replit ‚Ü?Lovable mapping (for Agents)

| Replit | Lovable |
|--------|---------|
| Replit Secrets | Lovable **Cloud ‚Ü?Secrets** (+ copy to Node host) |
...
`lovable-setup/1.0` ‚Ä?aligned with TomorrowOS protocol `1.0` and `@tomorrowos/sdk` store drivers `sqlite` | `supabase` | `postgres` | `memory`.

**Changelog 1.0:** Initial Lovable wizard mirroring REPLIT_SETUP Q1‚ÄìQ3; IRON RULE that Lovable Cloud Publish is not the WebSocket CMS host; Replit Object Storage replaced by Lovable Cloud Storage (+ Cloudinary still recommended for `/media/upload`).
